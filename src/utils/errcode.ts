...
export function errorMessageWithCode(code: string | number, value?: string) {
  const lang = i18n.global.locale.value
  // console.log('lang', lang, errCode[code])
  const message = errCode[code]?.[lang === 'cn' ? 'zh' : lang]
  if (!message) {
    console.warn('未查到此错误信息')
  }
  if (value) {
